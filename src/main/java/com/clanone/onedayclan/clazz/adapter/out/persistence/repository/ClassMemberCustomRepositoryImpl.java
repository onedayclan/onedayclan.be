...
import com.clanone.onedayclan.clazz.adapter.out.persistence.entity.QClassMemberEntity;
...
public class ClassMemberCustomRepositoryImpl implements ClassMemberCustomRepository{
    ...
    public ScheduledClassDetailModel getScheduledClassDetailModel(String userId, long classSeq) {
        QClassMemberEntity subClassMember = new QClassMemberEntity("subClassMember");
        return jpaQueryFactory.select(Projections.fields(ScheduledClassDetailModel.class,
                        classMemberEntity.attendanceCheck,
                        classReviewEntity.seq.as("reviewSeq"),
                        classMemberEntity.cancelYn,
                        classEntity.name.as("className"),
                        classEntity.teacherName.as("teacherName"),
                        classEntity.startAt,
                        classEntity.endAt,
                        classEntity.limitPeople,
                        ExpressionUtils.as(
                                JPAExpressions.select(subClassMember.seq.count())
                                        .from(subClassMember)
                                        .where(subClassMember.clazz.seq.eq(classSeq)),
                                "applicationPeople"),
                        ...
                        classMemberEntity.cancelMessage
                )).from(classMemberEntity)
                .leftJoin(classReviewEntity).on(classMemberEntity.clazz.seq.eq(classReviewEntity.clazz.seq))
                .leftJoin(classEntity).on(classMemberEntity.clazz.seq.eq(classEntity.seq))
                .leftJoin(memberEntity).on(classMemberEntity.member.seq.eq(memberEntity.seq))
                .where(memberEntity.userId.eq(userId).and(classEntity.seq.eq(classSeq)))
                .fetchOne();
    }
